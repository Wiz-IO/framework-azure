...
int mtk_os_hal_gpt_start(unsigned char timer_id)
{
	...
	ret = mtk_mhal_gpt_start(&(gpt_dev.hal_dev), (enum gpt_num)timer_id);
	if (ret) {
...
int mtk_os_hal_gpt_stop(unsigned char timer_id)
{
	...
	ret = mtk_mhal_gpt_stop(&(gpt_dev.hal_dev), (enum gpt_num)timer_id);
	if (ret)
		return ret;
...
int mtk_os_hal_gpt_reset_timer(unsigned char timer_id, unsigned int count_val,
			       bool auto_repeat)
{
	int ret = 0;
...
int mtk_os_hal_gpt_config(unsigned char timer_id, unsigned char speed_32us,
