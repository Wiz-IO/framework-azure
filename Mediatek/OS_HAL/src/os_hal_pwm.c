...
static struct mtk_pwm_controller_rtos *_mtk_os_hal_pwm_get_ctlr(int group_num)
{
	return &g_pwm_ctlr_rtos[group_num];
}

static int _mtk_os_hal_pwm_config_gpio(int group_num, u32 channel_bit_map)
{
	int ret = 0;
...
static int _mtk_os_hal_pwm_release_gpio(int group_num, u32 channel_bit_map)
{
	int ret = 0;
...
int mtk_os_hal_pwm_ctlr_init(int group_num, u32 channel_bit_map)
{
	...
	if (!ctlr_rtos->ctlr)
		return -EPTR;
	OS_DEBUG("mtk_os_hal_pwm_ctlr_init\n");
	ctlr = ctlr_rtos->ctlr;
		...
		struct mtk_com_pwm_data state_config)
{
	...
	ctlr = ctlr_rtos->ctlr;
	OS_DEBUG("state_config->frequency %d\n", state_config.frequency);
	OS_DEBUG("state_config->duty_cycle %d\n", state_config.duty_cycle);
	OS_DEBUG("state_config->stage %d\n", state_config.stage);
