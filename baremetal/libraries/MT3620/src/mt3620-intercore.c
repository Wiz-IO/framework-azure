...
#ifndef MAILBOX_BASE
#define MAILBOX_BASE 0x21050000
#endif
                ...
                uint32_t dataSize)
{
    ...
    *DataAreaOffset32(outbound, localWritePosition) = dataSize;
    ...
    memcpy(dest8, src8, writeToEnd);
    memcpy(DataAreaOffset8(outbound, 0), src8 + writeToEnd, dataSize - writeToEnd);
                ...
                uint32_t *dataSize)
{
    ...
    *dataSize = blockSize;
    ...
    uint8_t *dest8 = dest;
    memcpy(dest8, src8, readFromEnd);
    // If block wrapped around the end of the buffer, then read remainder from start.
    memcpy(dest8 + readFromEnd, DataAreaOffset8(inbound, 0), blockSize - readFromEnd);
